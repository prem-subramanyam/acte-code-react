...
function higherOrder() {
  ...
  const myInvestors = investors
    .map((investor) => {
      let eachInvestor = {};
      eachInvestor.name = investor.name;
      eachInvestor.role = investor.role;

      return eachInvestor;
    })
    .filter((truncInvestor) => {
      return truncInvestor.role.toLowerCase().includes("ceo");
    })
    .sort((inv1, inv2) => {
      return inv1.name.localeCompare(inv2.name);
    });

  console.log(myInvestors);
}
